...
Cookie是服务器发送到用户浏览器并保存在本地的一小块数据，会在下一次向同一服务器发送请求时被带上，用于告知服务器这两个请求是同一浏览器发出。由于之后每次请求都要带上Cookie，会带来额外的性能开销。
...
* Cookie存储在浏览器中，容易被恶意查看，如果非要在Cookie中存放一些隐私数据，可以对Cookie值进行加密，在服务端进行解密。
* 对于大型网站，如果所有用户信息都存储在Session中，开销将非常大，因此不建议将所有用户信息都存储在Session中，一般会将不常用的用户信息存储在数据库中，如MySQL。
