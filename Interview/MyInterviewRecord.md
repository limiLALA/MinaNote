...
第三次面试，我有点紧张，有些地方说的不是很好，面试官挺和蔼的，说准备做的很充分了，就不用紧张什么，问到不会的也不用慌，知识是学不完的，只要基础打好了就不用太过担心balabala~
...
HashTable与HasnMap的区别
Java反射
泛型什么时候会用到
索引什么时候会失效
数据库引擎有哪些
编程：将Excel列字母转换为实际数字，Excel用字母从A到Z命名它的列，顺序分别为A，B，C...Z，AA，AB，AC...AZ，BA，BB...等，列号分别对应1，2，3，4，5...；请编写一个函数，该函数接受字符串作为参数(如“AABCCE”)，并返回实际的列号；
思考：给你一瓶矿泉水，你要怎么测试（成分、水质等等）
最后的建议：有些问题给出来后可以先自己思考一下再讲出来，可能条理会更清晰一下。
## 虾皮QA测试HR面
1. 你喜欢怎样的一个工作环境
-----------------------------------------------
## 字节测开一面(今日头条/西瓜视频/内容生态)
#### 项目（智能车+腾讯实习）
#### 基础
##### 网络
TCP四次挥手
...
垃圾回收机制
#### 测试
用过抖音/微视吗？如果要测试视频的评论功能，要怎么测？
#### sql语句：student表中的列为id、name、score，请找出其中含有重复name的所有name，按name升序排列
```sql
select * from student where name in (select name from student group by name having count(name)>1) order by name;
```
#### 算法题
