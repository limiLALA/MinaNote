## 用户态和内核态
### 什么是用户态和内核态？
根据进程对资源访问的特点，可以把进程在OS上的运行分为2种级别
用户态：用户态的进程权限较低，只能操作原本分配给自己的内存空间，无法直接操作除此以外的内存、磁盘、外设、驱动、网络等，只能发起系统调用切换至内核态来访问这些敏感资源。
内核态：内核态的进程权限很高，几乎可以访问OS管理的所有资源，如内存空间、磁盘、文件系统、设备、驱动程序等。
我们能看到的所有应用程序大部分时间都运行在用户态，只有当进程需要执行一些OS底层的敏感操作时才需要切换至内核态。
有3种方式进入内核态，分别为系统调用/硬中断/异常(如缺页)。
以系统调用为例，切换过程为：用户态进程执行系统调用程序，触发软中断，CPU核将当前的内存数据保存到用户空间的寄存器后，将上下文切换到内核空间寄存器，并跳转到内核代码块执行。
需要注意的是，32位系统会给所有进程默认分配4GB虚拟内存空间，其中3GB为用户空间，1GB是内核空间。每个进程的内核空间是相互独立的，只有内核死代码是通过mmap的方式共享的。因此不用担心多个进程进入内核空间后会在内存中打架，但是涉及到外部资源的访问仍需谨慎关注并发冲突问题，如同时flush相同的磁盘文件。
### 不能只有内核态吗？
内核态的进程权限过大，能够直接操作所有的内存空间、磁盘、外设、网络，如果所有进程都运行在内核态，会导致并发冲突域太大，且能够执行所有危险的CPU特权指令，如内存分配、设置时钟、IO处理等，很容易产生系统安全问题。划分出用户态能够更好地控制进程权限，缩小冲突域，提升计算机系统安全性、稳定性和性能。

## 进程和线程
#### 什么是进程和线程？
进程是资源分配的基本单位，线程是处理器调度的基本单位
1个进程中至少要有1个线程，否则无法被处理器调度。
#### 进程和线程的区别是什么？
进程之间的资源是互斥的，无权限访问其他进程的内存空间，因此进程间通信必须通过内核。
一个进程运行期间可以创建很多个线程，这些线程之间大部分资源是共享的，通信无需进入内核态，开销更小，效率更高，但是资源访问冲突的概率也更大。
以JVM为例，进程与进程之间是完全独立的，同一个进程里的多个线程可以共享进程的堆和方法区，但每个线程又有自己私有的虚拟机栈、程序计数器和本地方法栈
#### 有了进程为什么还需要线程？
使用多线程能够更好地利用多核CPU，通过增加并发度来提升进程的执行效率。在部分线程发生IO阻塞时不影响其余线程继续工作。
